...
let MULtest stack = 
  let x,s = pop stack  //pop the top of the stack
  let y,s = pop s      //pop the result stack
  let r = x*y   //do the math 
  push r s     //push back on the doubly-popped stack
...
let unary f stack = 
  let x,stack' = pop stack  
  push (f x) stack'//f is a function

...
//PARAMETER: pass the element (elem) to be pushed 
 void stack::push(el_t elem){
   if (isFull()){
    throw Overflow();//throws an exception (Overflow)
   }
...
//PURPOSE: pops all elements from the stack to make it empty if it is not empty yet.
 void stack::clearIt(){
    if (isEmpty()){
     ...
     int z=0;//local var to send over the pop method
     for(int x:el){//loop threw all elements and remove them
     cout<<x<<endl;//** actualy remove
     try{
       pop(z);//pop using local var
     }
     catch(stack::Underflow){
       cout<<"Should never be called"<<endl;
       break;//should never be called but just in case
     }
    }
   }
